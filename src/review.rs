...
fn compute_next_srs_meta(fsrs_meta: &ReviewableFSRSMeta, resp: &ReviewResponse) -> Result<SRSMeta> {
    let next_fsrs_meta = compute_next_fsrs_meta(fsrs_meta, resp)?;
    let next_logseq_srs_meta = (&next_fsrs_meta).into();

    Ok(SRSMeta { logseq_srs_meta: next_logseq_srs_meta, fsrs_meta: next_fsrs_meta })
}
