...
fn build_typst_frontmatter(base_font_size_pt: u16, columns: u16, lines: u16, ppi: u16) -> String {
    let base_font_size_pt = base_font_size_pt as f32;
    let columns = (columns - 10) as f32;
    let lines = (lines - 5) as f32;

    let point_width = columns * (base_font_size_pt * 0.625);
    let inch_width = point_width / 72.0;

    let point_height = (lines * base_font_size_pt * 1.2);
    let inch_height = point_height / 72.0;

    let font_size = base_font_size_pt * 2.0;

    let mut rv = String::new();
    rv.push_str(&format!(
        "#set page(width: {}in, height: {}in, margin: {}pt)\n",
        inch_width, inch_height, font_size
    ));
    rv.push_str(&format!("#set text(size: {}pt)\n", font_size));
    rv.push_str(&format!(
        "#show quote: it => {{ rect( inset: (left: 12pt, rest: 8pt), stroke: (left: 3pt + gray, rest: none), it.body) }}\n",
    ));
...
fn typst_to_png(typst: Typst, graph_root: &Path) -> Result<Vec<u8>> {
    ...
    let typst_frontmatter = build_typst_frontmatter(12, 188, 52, 96);
    println!("{}", typst_frontmatter);
