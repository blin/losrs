...
impl RunLosrs {
    ...
    fn perform_step_in(&self, graph_root: &Path) -> Result<()> {
        ...
        assert_eq_text!(&self.expected_stdout, &String::from_utf8_lossy(&output.stdout));
        Ok(())
    }
}
...
fn test_file_inner(file_name: &str) -> Result<()> {
    let path =
        PathBuf::from(format!("{}/tests/txtestcases/{}", env!("CARGO_MANIFEST_DIR"), file_name));
    let archive = txtar::from_str(&fs::read_to_string(path)?);
