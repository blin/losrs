...
impl ReviewAction {
    fn dodo(&self, p: &mut PtySession) -> Result<()> {
        match self.name.as_str() {
            ...
            "send_control" => {
                p.send_control(self.args.chars().next().unwrap())?;
            }
            "flush" => {
                p.flush()?;
            }
...
impl RunLosrsReview {
    fn from_steps_dir(d: &Path, i: i32) -> Self {
        let action_args = step_data(d, i, "action_args").unwrap();
        let action_envs = step_data(d, i, "action_envs");
        let expected_exit_code = step_data(d, i, "expected_exit_code");
        let expected_exit_code: i32 =
            expected_exit_code.map(|s| s.trim_end().parse::<i32>().unwrap()).unwrap_or(0);
