...
fn parse_review_action(line: &str) -> Result<ReviewAction> {
    ...
    let action_name = review_action_name.trim_end();
    let action_args = review_action_args.trim_end_matches(|x| x == '\n');

    match action_name {
        "exp_string" => Ok(ReviewAction::ExpString(action_args.to_owned())),
        "send" => Ok(ReviewAction::Send(action_args.to_owned())),
        "send_control" => {
            let c = action_args
                .chars()
                .next()
                .ok_or_else(|| anyhow!("send_control requires a character argument"))?;
            Ok(ReviewAction::SendControl(c))
        }
        "flush" => Ok(ReviewAction::Flush),
        "read_line" => Ok(ReviewAction::ReadLine),
        _ => Err(anyhow!("unknown review action: {}", action_name)),
    }
}
